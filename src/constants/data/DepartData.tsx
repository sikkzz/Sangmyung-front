import Icons from "../icon";
import Img3 from "../../assets/profile/eduhead.jpeg";
import Img4 from "../../assets/profile/edusub.jpeg";
import Img5 from "../../assets/profile/planhead.jpeg";
import Img6 from "../../assets/profile/plansub.png";
import Img7 from "../../assets/profile/inhead.jpeg";
import Img8 from "../../assets/profile/insub.jpeg";
import Img9 from "../../assets/profile/fohead.jpeg";
import Img10 from "../../assets/profile/fosub.jpeg";
import Img11 from "../../assets/profile/welhead.jpeg";
import Img12 from "../../assets/profile/welsub.png";
import Img13 from "../../assets/profile/fihead.jpeg";
import Img14 from "../../assets/profile/fisub.jpg";
import Img15 from "../../assets/profile/prohead.jpg";
import Img16 from "../../assets/profile/prosub.png";
import Img17 from "../../assets/profile/songhee.jpeg";
import Img18 from "../../assets/profile/hyun.jpeg";
import Img19 from "../../assets/profile/gyuri.jpeg";
import Img20 from "../../assets/profile/gahyun.jpeg";
import Img21 from "../../assets/profile/minji.png";
import Img22 from "../../assets/profile/seogyung.png";
import Img23 from "../../assets/profile/kyungeun.jpeg";
import Img24 from "../../assets/profile/yeyoon.jpeg";
import Img25 from "../../assets/profile/jongmin.jpeg";
import Img26 from "../../assets/profile/yewon.jpeg";
import Img27 from "../../assets/profile/gyungjin.jpeg";
import Img28 from "../../assets/profile/hyejin.jpeg";
import Img29 from "../../assets/profile/soonghyeon.jpeg";
import Img30 from "../../assets/profile/jiwon.jpeg";
import Img31 from "../../assets/profile/jinhee.png";
import Img32 from "../../assets/profile/plan.png";
import Img33 from "../../assets/profile/welfare.png";
import Img34 from "../../assets/profile/education.png";
import Img35 from "../../assets/profile/finaicial.png";
import Img36 from "../../assets/profile/foreign.png";
import Img37 from "../../assets/profile/promotion.png";
import Img38 from "../../assets/profile/internal.png";

export const DepartIntroData = [
  {
    id: "planning",
    title: "기획국",
    sub: "Planning Department",
    contents:
      "상명대학교 서울캠퍼스 제 46대 총학생회 ‘선[先]’ 기획국입니다. 기획국에서는 축제, 체육대회, 동아리공연 등 다양한 행사를 기획하는 업무를 맡고 있습니다. 항상 재미있고 안전한 행사를 기획하며 즐거운 상명대학교를 만들기 위해 최선을 다해 노력하겠습니다.",
    img: Img32,
    alt: "img32",
  },
  {
    id: "financialofficework",
    title: "재정사무국",
    sub: "Financial officework Department",
    contents:
      "상명대학교 서울캠퍼스 제46대 총학생회 '선[先]' 재정사무국입니다. 재정사무국에서는 총학생회 예결산 처리 및 기안 작성, 공약 이행 사항 관리, 학내 청원 및 민원 관리, 회의록 및 기록물 관리 업무 등을 맡고 있습니다. 학우 여러분의 학생회비가 투명하게 쓰이도록 항상 노력하겠습니다.",
    img: Img35,
    alt: "img35",
  },
  {
    id: "foreigncooperation",
    title: "대외협력국",
    sub: "Foreign cooperation Department",
    contents:
      "상명대학교 서울캠퍼스 제46대 총학생회 '선[先]' 대외협력국입니다. 대외협력국에서는 각종 제휴 & 프로모션 협약 체결 업무, 외부기관과의 협력 체계 구축 등의 업무를 담당하고 있습니다. 학우 여러분의 더 나은 학교 생활을 위해 항상 노력하겠습니다.",
    img: Img36,
    alt: "img36",
  },
  {
    id: "internalcommunication",
    title: "대내교류국",
    sub: "Internal communication Department",
    contents:
      "상명대학교 서울캠퍼스 제46대 총학생회 '선[先]' 대내교류국입니다. 대내교류국은 상명대학교 서울캠퍼스 내부 소통을 담당하고 있습니다. 학우분들의 의견을 수렴하여, 학교에 학우분들의 의사가 반영될 수 있도록 항상 최선을 다해 노력하겠습니다.",
    img: Img38,
    alt: "img38",
  },
  {
    id: "welfarework",
    title: "복지사업국",
    sub: "Welfare work Department",
    contents:
      "상명대학교 서울캠퍼스 제46대 총학생회 '선[先]' 복지사업국입니다. 복지사업국은 상명대학교 전체 학우들의 복지를 위해 다양한 사업 혹은 활동을 지원하는 부서입니다. 대표적으로 농촌 봉사 활동을 기획하고 총괄하는 업무가 있으며 그 외에 의류 공동 사업, 사회 봉사 활동 등 학우분들의 만족스러운 학교 생활을 위한 전반적인 사항을 담당하고 있습니다.",
    img: Img33,
    alt: "img33",
  },
  {
    id: "educationsupporting",
    title: "교육지원국",
    sub: "Education supporting Department",
    contents:
      "상명대학교 서울캠퍼스 제46대 총학생회 '선[先]' 교육지원국입니다. 교육지원국은 학사운영 관련 사항 개선, 교육/진로/취업 등의 전보 전달, 국제교류 프로그램 정보 전달, 기타 교육지원을 위한 방안 모색 등을 맡고 있습니다. 학우 여러분이 유익한 학교 생활을 영위할 수 있도록 언제나 학사, 진로 등의 고민 앞에 한 걸음 먼저 서 있겠습니다.",
    img: Img34,
    alt: "img34",
  },
  {
    id: "promotion",
    title: "홍보국",
    sub: "Promotion Department",
    contents:
      "상명대학교 서울캠퍼스 제46대 총학생회 '선[先]' 홍보국입니다. 홍보국에서는 다양한 홍보물 제작을 통해 대・내외에 시각화 된 정보를 전달합니다. 총학생회 선[先]과 학우분들의 단단한 연결 다리가 되어 홍보물 전달에 최선을 다하겠습니다.",
    img: Img37,
    alt: "img37",
  },
];

export const DepartWorkData = [
  {
    id: "planning",
    work: [
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "총학생회 행사 기획",
        workContent:
          "대동제, 소동제, 체육대회 등  행사를 기획하여 상명대학교 학생들끼리의 유대관계를 향상시키며 안전하고 재미있는 행사를 운영 및 기획합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "총학생회 공약 사업 추진",
        workContent:
          "총학생회가 공약으로 추진시키려하는 행사 및 사업 내용을 기획하며 공약 이행률을 높이기 위해 노력합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "총학생회 방향성 제시",
        workContent:
          "관련된 부서에 방향성을 제시하고, 관련 부서에 운영을 통합하고 조정하며, 운영 가이드라인을 제공하는 역할을 합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "총학생회 사업 계획서 작성",
        workContent:
          "총학생회의 사업 계획서를 작성하고 제시합니다. 분기별 콘텐츠를 통해 학생회의 미션, 목표, 전략을 이행하기 위한 기획을 합니다",
      },
    ],
  },
  {
    id: "financialofficework",
    work: [
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "총학생회 기안 작성",
        workContent:
          "총학생회 예결산 처리 및 기안 작성을 통해 학생회비를 운용합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "공약 이행 사항 관리",
        workContent:
          "공약 이행 상황을 관리 및 게시하여 진행 상황을 학우 여러분께 알려드립니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "학내 청원 및 민원 관리",
        workContent:
          "학우 여러분의 불편한 점과 건의사항을 받아서 처리하고 관리합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "기록물 및 회의록 관리",
        workContent: "회의록과 여러가지 양식들을 작성 및 보관 관리합니다.",
      },
    ],
  },
  {
    id: "foreigncooperation",
    work: [
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "외부 업체와의 제휴 협약 체결",
        workContent:
          "각종 외부 업체와의 제휴 협약 체결을 통해 학우분들의 더 나은 학교 생활을 돕습니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "오프라인 상권 제휴 ",
        workContent:
          "상명대학교 서울캠퍼스 주변, 경복궁, 홍제, 종로 등 각종 오프라인 상권과의 제휴를 통해, 학우들에게 체감될 수 있는 혜택 제공을 도모합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "캠퍼스 이벤트 & 프로모션 유치",
        workContent:
          "외부 업체의 기간한정 캠퍼스 내 이벤트 & 프로모션 유치를 통해, 학우분들에게 더욱 다가갈 수 있도록 노력합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "SNS 홍보",
        workContent:
          "대외협력국이 맺는 모든 제휴/이벤트/프로모션이 학우 여러분께 알려질 수 있도록, SNS를 통한 카드뉴스 홍보를 진행합니다.",
      },
    ],
  },
  {
    id: "internalcommunication",
    work: [
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "학생 단체 및 교내 행정처 소통",
        workContent:
          "학우분들 및 교내 행정처와 소통을 통하여 의사를 듣고, 관련 사항을 개선하기 위해 활동합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "동아리 문화제 시행",
        workContent:
          "본교 문화의 중심인 동아리의 활성화를 위한 기획과 행사를 주관하여 진행합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "체전 및 SMCL",
        workContent:
          "교내 축구 리그 대회 주관 및 체전 전체적인 주관을 진행합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "교원 간담회",
        workContent:
          "학우분들의 의견이 학교로 더욱 정확하게 전달하기 위하여 소통을 중점적인 가치로서 항상 최선을 다하겠습니다.",
      },
    ],
  },
  {
    id: "welfarework",
    work: [
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "농촌 봉사 활동",
        workContent:
          "농촌의 일손 부족 문제를 해결하기 위한 농촌 봉사 활동을 기획하고 진행합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "의류 공동 사업",
        workContent: "전체 학우 대상 의류 공동 구매 사업을 추진합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "물품 대여 사업",
        workContent:
          "운영 경비 활용을 통한 기본 의약품 구비 및 물품 대여 사업 진행합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "학내 안전 관리",
        workContent:
          "학생회 내부 안전 관리 TF팀 구성하여 학우 여러분의 안전을 관리합니다.",
      },
    ],
  },
  {
    id: "educationsupporting",
    work: [
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "학사운영 관련 사항 개선",
        workContent:
          "전공제도, 수강신청 등 학우 여러분의 학교 생활과 밀접한 관련이 있는 학사운영에 대한 문제점을 파악하고 개선하고자 합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "교육, 진로, 취업 등 정보 전달",
        workContent:
          "학우 여러분이 유익한 학교 생활을 영위할 수 있도록 교내 학사 정보를 전달하고, 졸업 후 진출할 진로에 대한 방향 설정과 취업 준비에 필요한 정보를 전달합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "국제교류 프로그램 정보 전달",
        workContent:
          "타국에서 실용 외국어 능력을 향상을 위한 경험을 축적할 수 있는 교내 국제교류 프로그램의 정보를 전달합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "기타 교육지원을 위한 방안 모색",
        workContent:
          "교내 및 교외 교육 관련 활동(공모전, 특강 등)을 모색하고 학우 여러분께 알려드립니다.",
      },
    ],
  },
  {
    id: "promotion",
    work: [
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "총학생회 유튜브 계정 운영",
        workContent:
          "총학생회의 업무 모습 및 일상을 영상물을 통해 학우분들과 공유하여 총학생회에 대한 직관적인 이해를 돕고 심리적 거리를 좁힐 수 있도록 노력합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "학교 홍보 영상 제작",
        workContent:
          "학교 주요 행사와 교정의 모습을 영상물로 기록해 추후 외부에 학교를 홍보할 수 있는 자료들을 수집하고 게시합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "그래픽 홍보물 제작",
        workContent:
          "총학생회가 진행하는 행사 홍보물들을 제작하여 시각적으로 그 의미가 잘 전달될 수 있도록 합니다. 이를 위하여 그래픽 요소들을 이용한 카드 뉴스, 포스터 등을 제작합니다.",
      },
      {
        workIcon: <Icons.MdWorkOutline size={48} color="#fff" />,
        workTitle: "굿즈 및 디자인 그래픽 작업물 제작",
        workContent:
          "상명대학교와 총학생회의 정체성을 잘 투영하여 학교 및 축제 기념품, 공동 구매 의류 등 자체 제작 물품 및 굿즈등을 디자인합니다.",
      },
    ],
  },
];

export const DepartData = [
  {
    id: "planning",
    subItem: [
      {
        position: "국장",
        name: "박주현",
        major: "화학에너지공학전공 18",
        img: Img5,
        alt: "img5",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 기획국 국장 박주현입니다.<br/>상명대학교 학우분들이 즐길 수 있는 이벤트와 행사를 기획하며<br/>행복이 가득한 상명대학교를 만들기 위하여 노력하겠습니다. 감사합니다.",
      },
      {
        position: "차장",
        name: "이윤종",
        major: "행정학부 20",
        img: Img6,
        alt: "img6",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 기획국 차장 이윤종입니다.<br/>여러분들이 '기'대하는 대학생활을 '획'기적인 아이디어로<br/>아름답고 찬란하게 빛나도록 열심히 달리도록 하겠습니다!",
      },

      {
        position: "국원",
        name: "이주환",
        major: "휴먼지능정보공학전공 23",
        img: Img32,
        alt: "img2",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 기획국 국원 이주환입니다.",
      },
      {
        position: "국원",
        name: "이세린",
        major: "화학에너지공학전공 21",
        img: Img32,
        alt: "img2",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 기획국 국원 이세린입니다.",
      },
      {
        position: "국원",
        name: "김진희",
        major: "가족복지학과 22",
        img: Img31,
        alt: "img31",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 기획국 국원 김진희입니다.<br/>학우분들의 행복한 추억 속 일부가 될 수 있도록<br/>재미있는 축제, 행사 기획에 힘쓰겠습니다! ",
      },
      {
        position: "국원",
        name: "연하늘",
        major: "스포츠건강관리전공 23",
        img: Img32,
        alt: "img2",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 기획국 국원 연하늘입니다.",
      },
      {
        position: "국원",
        name: "이종민",
        major: "경영학부 22",
        img: Img25,
        alt: "img25",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 기획국 국원 이종민입니다.<br/>학우분들이 훗날 상명대를 떠올렸을 때 행복했던 추억이 많았다고<br/>느낄 수 있도록 노력하겠습니다!",
      },
      {
        position: "국원",
        name: "남윤규",
        major: "전기공학전공 22",
        img: Img32,
        alt: "img2",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 기획국 국원 남윤규입니다.",
      },
      {
        position: "국원",
        name: "정예원",
        major: "문헌정보학전공 23",
        img: Img26,
        alt: "img26",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 기획국 국원 정예원입니다.<br/>기획국원으로서 국차장님과 함께 열심히 일하겠습니다💙",
      },
      {
        position: "국원",
        name: "문예윤",
        major: "게임전공 21",
        img: Img24,
        alt: "img24",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 기획국 국원 문예윤입니다.<br/>열정열정열정!",
      },
      {
        position: "국원",
        name: "김민지",
        major: "경영학부 23",
        img: Img21,
        alt: "img21",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 기획국 국원 김민지입니다.<br/>단조로운 학교생활 속 다양한 즐거움을 드리기 위해 최선을 다하겠습니다!",
      },
    ],
  },
  {
    id: "financialofficework",
    subItem: [
      {
        position: "국장",
        name: "김준식",
        major: "컴퓨터과학전공 17",
        img: Img13,
        alt: "img13",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 재정사무국 국장 김준식입니다.<br/>학우 여러분에게 희망과 행복이 가득한 청춘의 대학생활과 더불어<br/>학우들을 위한, 학우들이 원하는 캠퍼스를 만들고<br/>따뜻한 봄과 함께 새로운 시작이 도래할 수 있도록 노력하겠습니다.",
      },
      {
        position: "차장",
        name: "김지훈",
        major: "화공신소재전공 19",
        img: Img14,
        alt: "img14",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 재정사무국 차장 김지훈입니다.<br/>이번년도 학우 여러분과 학생회 사이의 다리가 되어<br/>불편함 없도록 노력하겠습니다.",
      },
      {
        position: "국원",
        name: "김경은",
        major: "행정학부 20",
        img: Img23,
        alt: "img2",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 재정사무국 국원 김경은입니다.<br/>투명한 학생회가 될 수 있도록 노력하겠습니다.",
      },
      {
        position: "국원",
        name: "백지연",
        major: "휴먼지능정보공학전공 22",
        img: Img35,
        alt: "img35",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 재정사무국 국원 백지연입니다. ",
      },
      {
        position: "국원",
        name: "김남경",
        major: "휴먼지능정보공학전공 22",
        img: Img35,
        alt: "img35",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 재정사무국 국원 김남경입니다.<br/>1년동안 상명대학교 학우분들에게 많은 경험과 좋은 기억들을<br/>쌓을 수 있도록 최선을 다하겠습니다.",
      },
      {
        position: "국원",
        name: "서찬솔",
        major: "전기공학전공 22",
        img: Img35,
        alt: "img35",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 재정사무국 국원 서찬솔입니다",
      },
      {
        position: "국원",
        name: "이현승",
        major: "화공신소재전공 22",
        img: Img35,
        alt: "img35",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 재정사무국 국원 이현승입니다",
      },
      {
        position: "국원",
        name: "강하연",
        major: "영어교육과 23",
        img: Img35,
        alt: "img35",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 재정사무국 국원 강하연입니다",
      },
      {
        position: "국원",
        name: "이규리",
        major: "가족복지학과 22",
        img: Img19,
        alt: "img19",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 재정사무국 국원 이규리입니다.<br/>학우분들의 대학생활이 온화한 바람으로 가득 채워지도록<br/>하나의 입김이 되어드리겠습니다.",
      },
    ],
  },
  {
    id: "foreigncooperation",
    subItem: [
      {
        position: "국장",
        name: "이지훈",
        major: "역사콘텐츠전공 18",
        img: Img9,
        alt: "img9",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 대외협력국 국장 이지훈입니다.<br/>대외협력국은 외부와의 제휴, 협약, 프로모션 등의 진행을 담당하고 있습니다.<br/>1년 동안 여러분들에게 도움이 될 모든 내용들을 '대'담하게 '외'치고 다니겠습니다.<br/>많은 관심 부탁드립니다. 감사합니다",
      },
      {
        position: "차장",
        name: "장윤서",
        major: "행정학부 22",
        img: Img10,
        alt: "img10",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 대외협력국 차장 장윤서입니다.<br/>소중한 목소리들을 최대한 반영하고 양질의 학교 생활을<br/>선물해 드릴 수 있도록 노력하겠습니다.",
      },
      {
        position: "국원",
        name: "김건태",
        major: "컴퓨터과학전공 18",
        img: Img36,
        alt: "img36",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대외협력국 국원 김건태입니다.",
      },
      {
        position: "국원",
        name: "류민우",
        major: "화학에너지공학전공 18",
        img: Img36,
        alt: "img36",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대외협력국 국원 류민우입니다.",
      },
      {
        position: "국원",
        name: "이문언",
        major: "화학에너지공학전공 18",
        img: Img36,
        alt: "img36",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대외협력국 국원 이문언입니다.",
      },
      {
        position: "국원",
        name: "이지수",
        major: "화학에너지공학전공 20",
        img: Img36,
        alt: "img36",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대외협력국 국원 이지수입니다.",
      },
      {
        position: "국원",
        name: "한서연",
        major: "화학에너지공학전공 21",
        img: Img36,
        alt: "img36",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대외협력국 국원 한서연입니다.",
      },
      {
        position: "국원",
        name: "박서경",
        major: "한일문화콘텐츠전공 22",
        img: Img22,
        alt: "img22",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 대외협력국 국원 박서경입니다.<br/>열심히 활동하겠습니다. 감사합니다 !",
      },
      {
        position: "국원",
        name: "허유경",
        major: "역사콘텐츠전공 20",
        img: Img36,
        alt: "img36",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대외협력국 국원 허유경입니다.",
      },
      {
        position: "국원",
        name: "김지은",
        major: "경영학부 20",
        img: Img36,
        alt: "img36",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대외협력국 국원 김지은입니다.",
      },
      {
        position: "국원",
        name: "이하정",
        major: "경제금융학부 20",
        img: Img36,
        alt: "img36",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대외협력국 국원 이하정입니다.",
      },
      {
        position: "국원",
        name: "최민",
        major: "융합경영학과 23",
        img: Img36,
        alt: "img36",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 대외협력국 국원 최민입니다.<br/>학우분들이 만족스러운 학생 사회를 경험하고<br/>더 많은 혜택을 누릴 수 있도록 노력하겠습니다.",
      },
    ],
  },
  {
    id: "internalcommunication",
    subItem: [
      {
        position: "국장",
        name: "우윤기",
        major: "지적재산권전공 20",
        img: Img7,
        alt: "img7",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 대내교류국 국장 우윤기입니다.<br/>상명대학교 학우분들과 소통, 교류하여 항상 한걸음 앞에 있는<br/>총학생회가 될 수 있도록 최선을 다하겠습니다.",
      },
      {
        position: "차장",
        name: "하다현",
        major: "국어교육과 22",
        img: Img8,
        alt: "img8",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 대내교류국 차장 하다현입니다.<br/>학우분들의 소리에 귀 기울이며 원활한 소통으로<br/>학우분들의 소리가 닿을 수 있도록 최선을 다하겠습니다.",
      },
      {
        position: "국원",
        name: "방호진",
        major: "지적재산권전공 20",
        img: Img38,
        alt: "img38",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대내교류국 국원 방호진입니다.",
      },
      {
        position: "국원",
        name: "이서현",
        major: "경영학부 22",
        img: Img38,
        alt: "img38",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대내교류국 국원 이서현입니다.",
      },
      {
        position: "국원",
        name: "박지원",
        major: "가족복지학과 22",
        img: Img30,
        alt: "img30",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 대내교류국 국원 박지원입니다.<br/>학생들의 편안하고 즐거운 학교생활을 위해 최선을 다하겠습니다. 감사합니다.",
      },
      {
        position: "국원",
        name: "정세인",
        major: "행정학부 22",
        img: Img38,
        alt: "img38",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대내교류국 국원 정세인입니다.",
      },
      {
        position: "국원",
        name: "원예영",
        major: "글로벌경영학과 22",
        img: Img38,
        alt: "img38",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 대내교류국 국원 원예영입니다.<br/>학우분들의 의견을 기반으로 소통하며 더 나은 학교를 만들 수 있도록<br/>최선을 다하겠습니다. 그리고 학우분들의 더 나은 2023년<br/>상명대 생활을 위해 열심히 노력하겠습니다. 감사합니다!",
      },
      {
        position: "국원",
        name: "오동환",
        major: "화학에너지공학전공 18",
        img: Img38,
        alt: "img38",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대내교류국 국원 오동환입니다.",
      },
      {
        position: "국원",
        name: "김민정",
        major: "국어교육과 23",
        img: Img38,
        alt: "img38",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 대내교류국 국원 김민정입니다.",
      },
      {
        position: "국원",
        name: "김혜진",
        major: "행정학부 23",
        img: Img28,
        alt: "img28",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 대내교류국 국원 김혜진입니다.<br/>학우분들의 만족스러운 학교생활에 조금이나마<br/>도움이 될 수 있도록 노력하겠습니다!",
      },
      {
        position: "국원",
        name: "조경진",
        major: "문헌정보학전공 23",
        img: Img27,
        alt: "Img27",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 대내교류국 국원 조경진입니다.<br/>좀 더 발전된 상명대학교를 학우분들이 누릴 수 있도록<br/>최선을 다해 노력하겠습니다!",
      },
    ],
  },
  {
    id: "welfarework",
    subItem: [
      {
        position: "국장",
        name: "김성미",
        major: "전기공학전공 19",
        img: Img11,
        alt: "img11",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 복지사업국 국장 김성미입니다.<br/>복지사업국은 학우들이 다양한 활동과 혜택을 누릴 수 있도록<br/>기획하고 총괄하는 부서입니다. 상명대학교 학우분들이<br/>다양한 경험과 추억을 쌓을 수 있도록 항상 최선을 다하겠습니다.",
      },
      {
        position: "차장",
        name: "김연준",
        major: "식품영양학전공 20",
        img: Img12,
        alt: "img12",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 복지사업국 차장 김연준입니다.<br/>상명대학교 학우들의 소리를 듣고 발로 뛰어 2023년이<br/>학우들에게 좋은 기억들로만 남을 수 있도록 노력하겠습니다.",
      },
      {
        position: "국원",
        name: "석준호",
        major: "글로벌경영학과 20",
        img: Img33,
        alt: "img33",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 복지사업국 국원 석준호입니다.<br/>학우분들의 복지와 편의를 위해 총학생회의 일원으로 항상 최선을 다하겠습니다.",
      },
      {
        position: "국원",
        name: "정여진",
        major: "휴먼지능정보공학전공 22",
        img: Img33,
        alt: "img33",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 복지사업국 국원 정여진입니다.",
      },
      {
        position: "국원",
        name: "문채린",
        major: "휴먼지능정보공학전공 22",
        img: Img33,
        alt: "img33",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 복지사업국 국원 문채린입니다.<br/>상명대학교 학우분들이 좋은 추억을 쌓을 수 있도록 열심히 노력하겠습니다!",
      },
      {
        position: "국원",
        name: "서송희",
        major: "한일문화콘텐츠전공 22",
        img: Img17,
        alt: "img17",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 복지사업국 국원 서송희입니다.<br/>학우분들의 다양한 경험과 기회를 위해 노력하겠습니다 :)",
      },
      {
        position: "국원",
        name: "조민재",
        major: "역사콘텐츠전공 20",
        img: Img33,
        alt: "img33",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 복지사업국 국원 조민재입니다.",
      },
      {
        position: "국원",
        name: "배원혁",
        major: "국가안보학과 20",
        img: Img33,
        alt: "img33",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 복지사업국 국원 배원혁입니다.",
      },
      {
        position: "국원",
        name: "오민기",
        major: "국가안보학과 20",
        img: Img33,
        alt: "img33",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 복지사업국 국원 오민기입니다.",
      },
      {
        position: "국원",
        name: "이현지",
        major: "행정학부 22",
        img: Img33,
        alt: "img33",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 복지사업국 국원 이현지입니다.",
      },
      {
        position: "국원",
        name: "김가현",
        major: "글로벌경영학과 22",
        img: Img20,
        alt: "img20",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 복지사업국 국원 김가현입니다. 학생의 복지를 위해서 최선을 다하는 사람이 되겠습니다.",
      },
      {
        position: "국원",
        name: "이건호",
        major: "국가안보학과 23",
        img: Img33,
        alt: "img33",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 복지사업국 국원 이건호입니다.",
      },
      {
        position: "국원",
        name: "양현정",
        major: "스포츠건강관리전공 21",
        img: Img33,
        alt: "img33",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 복지사업국 국원 양현정입니다.",
      },
    ],
  },
  {
    id: "educationsupporting",
    subItem: [
      {
        position: "국장",
        name: "문수민",
        major: "지적재산권전공 20",
        img: Img3,
        alt: "img3",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 교육지원국 국장 문수민입니다.<br/>학우 여러분의 학사, 진로, 취업과 같은 고민이 있으면<br/>귀를 기울이고 더 나은 학교생활을 제공하기 위하여 최선을 다하겠습니다.<br/>선[先]의 다양한 활동에 많은 관심과 응원 부탁드립니다. 감사합니다!",
      },
      {
        position: "차장",
        name: "조다성",
        major: "식품영양학전공 22",
        img: Img4,
        alt: "img4",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 교육지원국 차장 조다성입니다.<br/>학우들이 교육적으로 불편한점이 없도록 항상 한발자국 먼저 움직여<br/>선[先] 을 실천하도록 하겠습니다. 상명대 선 조다성 Let’s go!",
      },
      {
        position: "국원",
        name: "김도현",
        major: "화학에너지공학전공 18",
        img: Img34,
        alt: "img34",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 교육지원국 국원 김도현입니다.",
      },
      {
        position: "국원",
        name: "오승현",
        major: "게임전공 18",
        img: Img29,
        alt: "img29",
        size: "squ",
        content:
          "안녕하십니까, 총학생회 선[先] 교육지원국 국원 오승현입니다.<br/>대학생활의 마지막 챕터를 여러분들과의 추억으로 채우고 싶습니다 :)",
      },
      {
        position: "국원",
        name: "이영현",
        major: "행정학부 19",
        img: Img18,
        alt: "img18",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 교육지원국 국원 이영현입니다.<br/>나 이영현이야",
      },
      {
        position: "국원",
        name: "조정현",
        major: "휴먼지능정보공학전공 20",
        img: Img34,
        alt: "img34",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 교육지원국 국원 조정현입니다.",
      },
      {
        position: "국원",
        name: "최민준",
        major: "컴퓨터과학전공 20",
        img: Img34,
        alt: "img34",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 교육지원국 국원 최민준입니다.",
      },
      {
        position: "국원",
        name: "나희진",
        major: "화공신소재전공 21",
        img: Img34,
        alt: "img34",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 교육지원국 국원 나희진입니다.",
      },
      {
        position: "국원",
        name: "한세민",
        major: "화학에너지공학전공 21",
        img: Img34,
        alt: "img34",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 교육지원국 국원 한세민입니다.",
      },
      {
        position: "국원",
        name: "이서준",
        major: "컴퓨터과학전공 22",
        img: Img34,
        alt: "img34",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 교육지원국 국원 이서준입니다.",
      },
    ],
  },
  {
    id: "promotion",
    subItem: [
      {
        position: "국장",
        name: "원가희",
        major: "의류학전공 21",
        img: Img15,
        alt: "img15",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 홍보국 국장 원가희입니다.<br/>일을 아무리 잘 하더라도 학우분들에게 닿지 않는다면<br/>아무런 의미가 없습니다. 총학생회 선[先]과 상명대학교<br/>학우분들의 단단한 연결 다리가 되겠습니다.",
      },
      {
        position: "차장",
        name: "김슬찬",
        major: "역사콘텐츠전공 22",
        img: Img16,
        alt: "img16",
        size: "rec",
        content:
          "안녕하십니까, 총학생회 선[先] 홍보국 차장 김슬찬입니다.<br/>긴 공백 끝에 제46대 총학생회 선[先]이 학우분들의 성원 덕분에 출범했습니다.<br/>홍보국 차장으로서 2023년 한 해동안 펼쳐질 상명대학교가 학교 구성원들에게,<br/>관심을 가져주시는 모든 분들에게 닿을 수 있도록 힘쓰겠습니다.",
      },
      {
        position: "국원",
        name: "신성빈",
        major: "글로벌경영학과 22",
        img: Img37,
        alt: "img37",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 홍보국 국원 신성빈입니다.",
      },
      {
        position: "국원",
        name: "지동근",
        major: "화학에너지공학전공 18",
        img: Img37,
        alt: "img37",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 홍보국 국원 지동근입니다.",
      },
      {
        position: "국원",
        name: "임경환",
        major: "국가안보학과 20",
        img: Img37,
        alt: "img37",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 홍보국 국원 임경환입니다.",
      },
      {
        position: "국원",
        name: "김민정",
        major: "국어교육과 22",
        img: Img37,
        alt: "img37",
        size: "squ",
        content: "안녕하십니까, 총학생회 선[先] 홍보국 국원 김민정입니다.",
      },
    ],
  },
];

// export const DepartPromissData = [
//   {
//     id: "planning",
//     subItem: [
//       {
//         icon: <Icons.MdFestival size={80} color="#000" />,
//         title: "대동제 기획",
//         progress: "ing"
//       },
//       {
//         icon: <Icons.MdOutlineFestival size={80} color="#000" />,
//         title: "동아리 문화제 시행",
//         progress: "ing"
//       },
//       {
//         icon: <Icons.FaMedal size={80} color="#000" />,
//         title: "체전 시행 및 SMCL 재활성화",
//         progress: "ing"
//       },
//       {
//         icon: <Icons.GiPartyPopper size={80} color='#000' />,
//         title: "학기 중 이벤트성 행사 진행",
//         progress: "ing"
//       }
//       // {
//       //   icon: <Icons.
//       // }
//     ]
//   },
//   {
//     id: "financialofficework",
//     subItem: [
//       {
//         icon: <Icons.BsUiChecksGrid size={80} color="#000" />,
//         title: "공약 이행 점검",
//         progress: "ing"
//       },
//       {
//         icon: <Icons.AiFillFolderOpen size={80} color="#000" />,
//         title: "투명한 학생회비 집행 내역 공개",
//         progress: "ing"
//       },
//     ]
//   }
// ]
